...
#
# AUTHENTIFICATION :
#   Header requis : X-API-Key: ids-pme-2024
...
API_KEY_VALUE     = os.getenv("IDS_API_KEY", "ids-pme-2024")
...
def verify_api_key(api_key: str = Depends(api_key_header)):
    ...
    if api_key != API_KEY_VALUE:
        logger.warning(f"Tentative d'acces avec cle invalide : {api_key}")
        raise HTTPException(
            status_code=403,
            detail="Cle API invalide. Header requis : X-API-Key: ids-pme-2024"
        )
    return api_key
...
**Authentification** : Header `X-API-Key: ids-pme-2024` requis sur tous les endpoints sauf `/health`.
...
def reset_history(api_key: str = Depends(verify_api_key)):
