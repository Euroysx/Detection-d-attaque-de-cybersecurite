...
def send_to_ids(metrics: dict, attack_label: str) -> dict:
    ...
    try:
        r = requests.post(API_URL, json=metrics, timeout=3)
...
Exemples :
  ...
  sudo python real_traffic_demo.py --demo scan
    ...
    print(f"{CYAN}Boucle infinie — Ctrl+C pour arreter{RESET}\n")

    loop_count = 0
    try:
        while True:
            loop_count += 1
            print(f"{BOLD}--- Cycle #{loop_count} ---{RESET}")
            demo_map[args.demo]()
            time.sleep(1)
    except KeyboardInterrupt:
        print(f"\n{YELLOW}Arrete apres {loop_count} cycle(s).{RESET}")
