...
if "📊 Dashboard" in page:
    ...
    <div class="main-header">
        ...
        <p style="margin:5px 0 0 0; opacity:0.8;">Système de Détection d'Intrusions | By Euroysx | XGBoost</p>
    </div>
    """, unsafe_allow_html=True)
...
# ================================================================
    ...
    with st.form("flow_form"):
        ...
        with col1:
            ...
            dest_port = st.number_input("Port destination", 0, 65535, 80)

        with col2:
            st.markdown("**Métriques de flux**")
            flow_duration = st.number_input("Durée du flux (µs)", 0.0, 1e9, 38308.0)
            fwd_packets   = st.number_input("Paquets forward", 0.0, 1e6, 1.0)
            bwd_packets   = st.number_input("Paquets backward", 0.0, 1e6, 1.0)
            flow_bytes    = st.number_input("Débit octets/s", 0.0, 1e9, 156.9)
            flow_pkts     = st.number_input("Débit paquets/s", 0.0, 1e6, 52.2)

        with col3:
            st.markdown("**Métriques paquets**")
            avg_pkt_size  = st.number_input("Taille moy. paquet", 0.0, 65535.0, 6.0)
            pkt_len_mean  = st.number_input("Longueur moy. paquet", 0.0, 65535.0, 6.0)
            win_fwd       = st.number_input("Fenêtre TCP forward", -1.0, 65535.0, 65535.0)
            win_bwd       = st.number_input("Fenêtre TCP backward", -1.0, 65535.0, 65535.0)
            iat_mean      = st.number_input("IAT moyen", 0.0, 1e9, 38308.0)
            iat_std       = st.number_input("IAT écart-type", 0.0, 1e9, 0.0)
        ...
        scenario = st.selectbox(
            "Charger un scénario :",
            ["-- Personnalisé --", "Trafic normal (HTTP)", "DDoS simulé",
             "PortScan simulé", "SSH Brute Force"]
        )

        submitted = st.form_submit_button("🔍 Analyser ce flux", use_container_width=True)

    # Remplissage des scénarios (hors formulaire)
    scenarios_data = {
        "Trafic normal (HTTP)": {
            "destination_port": 80, "flow_duration": 38308,
        ...
        "DDoS simulé": {
            "destination_port": 80, "flow_duration": 500,
            "total_fwd_packets": 5000, "total_backward_packets": 0,
            "flow_bytes_per_s": 9999999, "flow_packets_per_s": 999999,
            "average_packet_size": 60.0, "packet_length_mean": 60.0,
            "init_win_bytes_forward": 1024, "init_win_bytes_backward": 0,
            "flow_iat_mean": 0.1, "flow_iat_std": 0.01,
        },
        "PortScan simulé": {
            "destination_port": 22, "flow_duration": 3,
            "total_fwd_packets": 2, "total_backward_packets": 0,
            "flow_bytes_per_s": 4000, "flow_packets_per_s": 666666,
            "average_packet_size": 6.0, "packet_length_mean": 6.0,
            "init_win_bytes_forward": 1024, "init_win_bytes_backward": -1,
            "flow_iat_mean": 3.0, "flow_iat_std": 0.0,
        },
        "SSH Brute Force": {
            "destination_port": 22, "flow_duration": 479,
            "total_fwd_packets": 11, "total_backward_packets": 5,
            "flow_bytes_per_s": 1047.0, "flow_packets_per_s": 33.4,
            "average_packet_size": 49.8, "packet_length_mean": 49.8,
            "init_win_bytes_forward": 65535, "init_win_bytes_backward": 65535,
            "flow_iat_mean": 43.5, "flow_iat_std": 15.2,
        },
    }

    if submitted:
        # Si scénario sélectionné, utiliser ses valeurs
        if scenario != "-- Personnalisé --" and scenario in scenarios_data:
            payload = {**scenarios_data[scenario],
